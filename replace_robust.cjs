...
const startTag = 'const ShotMapPitch = (';
...
if (startIdx !== -1) {
  ...
  if (endIdx !== -1) {
    // Add missing imports too
    if (!content.includes('ChevronLeft')) {
        content = content.replace(
            "import { ChevronDown, Calendar, Loader2, PlayCircle, Trophy, Activity, Shirt, MapPin } from 'lucide-react';",
            "import { ChevronDown, Calendar, Loader2, PlayCircle, Trophy, Activity, Shirt, MapPin, ChevronLeft, ChevronRight } from 'lucide-react';"
        );
    }

    content = content.substring(0, startIdx) + newComponents + content.substring(endIdx);
    fs.writeFileSync(path, content, 'utf8');
    console.log('Replaced ShotMapPitch successfully.');
  } else {
    console.log('Could not find end of ShotMapPitch component.');
  }
} else {
  console.log('Could not find ShotMapPitch component.');
}
