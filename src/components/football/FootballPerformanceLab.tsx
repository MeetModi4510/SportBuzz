...
export function FootballPerformanceLab({ activePlayer }: FootballPerformanceLabProps) {
    if (!activePlayer) {
        return (
            <div className="flex-1 flex flex-col items-center justify-center dark:border-white/10 border-slate-200 rounded-[2.5rem] dark:bg-white/[0.02] bg-white backdrop-blur-[60px] dark:text-slate-500 text-slate-600 min-h-[600px] shadow-[0_8px_32px_0_rgba(31,38,135,0.37)] relative overflow-hidden group w-full z-20">
                ...
                <div className="relative flex items-center justify-center mb-10">
                    ...
                    <div className="w-24 h-24 dark:bg-white/5 bg-slate-50 backdrop-blur-xl rounded-full flex items-center justify-center border dark:border-white/20 border-slate-200 shadow-[0_0_40px_rgba(16,185,129,0.5)] transform rotate-3 group-hover:rotate-0 transition-transform duration-500 relative z-10">
    ...
    if (isLoading) {
        return (
            <div className="flex-1 flex flex-col items-center justify-center min-h-[600px] w-full relative z-20">
                ...
                <h3 className="text-xl font-bold dark:text-white text-slate-800 mb-2">Analyzing Player Data...</h3>
                <p className="dark:text-gray-400 text-slate-600 text-sm">Fetching live statistics, traits, and shot maps from FotMob</p>
            </div>
        );
    }
    ...
    if (error || !profile) {
        return (
            <div className="flex-1 flex flex-col items-center justify-center min-h-[600px] w-full relative z-20 text-center">
                <div className="w-20 h-20 dark:bg-red-500/10 bg-red-50 border dark:border-red-500/20 border-red-200 rounded-full flex items-center justify-center mb-6">
                   <span className="text-red-400 text-3xl font-black">!</span>
                </div>
                <h3 className="text-xl font-bold dark:text-white text-slate-800 mb-2">Data Unavailable</h3>
                <p className="dark:text-gray-400 text-slate-600 text-sm max-w-md">Could not fetch detailed performance data for {player.name}. The API might be rate limited or the player ID is invalid.</p>
            </div>
        );
    }
