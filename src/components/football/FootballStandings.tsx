...
async function fetchStandings(): Promise<StandingsResponse> {
  const res = await fetch(`${API_BASE}/api/football/standings`);
...
function getQualStyle(qualColor: string | null): {
  ...
  if (q.includes("champions") || q.includes("ucl"))
    return { bar: "bg-sky-500", row: "bg-sky-500/5 hover:bg-sky-500/10", badge: "bg-sky-500/20 text-sky-400 border-sky-500/30", label: "Champions League" };
  ...
  return (
    <section className="space-y-5">
      ...
      <div className="flex items-center gap-3 px-2">
        ...
        <div>
          <h2 className="text-xl font-bold tracking-tight text-white/90">Premier League Standings</h2>
          {data?.lastFetched && (
            <p className="text-xs text-white/30 mt-0.5">
              Last updated · {format(new Date(data.lastFetched), "dd MMM yyyy, HH:mm")}
            </p>
          )}
        </div>
      </div>
      ...
      {data?.data && data.data.length > 0 && (
        <div className="rounded-2xl border border-white/8 bg-white/[0.03] backdrop-blur-sm overflow-hidden">
          ...
          <div className="flex items-center gap-5 px-5 py-3 border-b border-white/5 text-[10px] font-semibold text-white/30 uppercase tracking-widest">
            <span className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-sky-500 inline-block" /> Champions League</span>
                  ...
                  return (
                    <tr
                      ...
                      className={cn(
                        ...
                        q.row || "hover:bg-white/5"
                      )}
                    >
                      {/* Qualification colour bar + Position */}
                      <td className="pl-0 pr-3 py-3">
                        <div className="flex items-center gap-0">
                          <span className={cn("w-1 h-8 rounded-r-full flex-shrink-0", q.bar)} />
                          <span className="pl-3 text-white/40 font-bold text-xs w-6 text-right">
                            {row.position}
                          </span>
                        </div>
                      </td>
                      ...
                      <td className="px-3 py-3">
                        <div className="flex items-center gap-2.5">
                          ...
                          <div>
                            <p className="font-semibold text-white/90 leading-tight whitespace-nowrap">
                              {row.teamName}
                            </p>
          ...
          <div className="px-5 py-3 border-t border-white/5 flex justify-between items-center">
            ...
            <span className="text-[10px] text-white/20 font-semibold">Premier League 2025/26</span>
          </div>
        </div>
      )}
    </section>
